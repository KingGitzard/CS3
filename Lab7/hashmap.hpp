...
// Mikhail Nesterenko: adapted from Proc C++, modified by Hunter Harbison
// 3/15/2022
...
public:
   ...
   void rehash(size_t n);            // element with key, inserts if does not exist
private:
...
pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool>  hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
   ...
   typename list<pair<const Key, Value>>::iterator it= findElement(x.first, bucket);    
   if (it == elems_[bucket].end()) {
      ++size_;
    ...
    --size_;
      if(size_ == 0){
		...
		if(++it != elems_[bucket].end()){ 
			...
			int bucket1 = bucket + 1;
			if(bucket1 >= hash_.numBuckets()) bucket1 = 0;
			while(elems_[bucket1].empty()) ++bucket1;
			return make_pair(&(*elems_[bucket1].begin()), true);
		}
	}
    else{ 
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
  ...
  if (found == nullptr) {
    auto it = insert(make_pair(x, Value()));
    return *(it.first);
  }
  return found->second;
}
...
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n){
  ...
  hash_ = DefaultHash<Key>(n);
	...
	for(auto& list : elems_){
		for(auto& e : list) tmp.push_back(e);
	}
	elems_.empty();
	elems_ = vector<list<Element>>(hash_.numBuckets());
	for(auto& c : tmp){
		size_t b = hash_.hash(c.first);
		elems_[b].push_back(c);
	}
}